  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :engagement/intake :subject "eng-1"
                   :patch {:id "eng-1" :operator "Yaren Fisheries Cooperative"}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= "Yaren Fisheries Cooperative" (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest procurement-agent-requirement-mismatch-is-held-and-unoverridable
  (testing "declared contract-value above $3,000 but not conducted via an authorised procurement agent -> HARD hold (flagship check part 1)"
    (let [[db actor] (fresh)
      ...
      (is (some #{:procurement-agent-requirement-mismatch} (-> (store/ledger db) last :basis)))
      (is (empty? (store/submit-history db))))))

(deftest artificial-division-violation-is-held-and-unoverridable
  (testing "two engagements sharing a split-group, each individually below $3,000, whose sum crosses it and neither via a procurement agent -> HARD hold (flagship check part 2, cross-entity)"
    (let [[db actor] (fresh)
          _ (assess! actor "t6apre" "eng-6")
          _ (draft! actor "t6apre" "eng-6")
          _ (assess! actor "t6bpre" "eng-7")
          _ (draft! actor "t6bpre" "eng-7")
          res (exec-op actor "t6" {:op :filing/submit :subject "eng-6"} operator)]
      (is (= :hold (get-in res [:state :disposition])) "settles immediately, no interrupt")
      (is (not= :interrupted (:status res)))
      (is (some #{:artificial-division-violation} (-> (store/ledger db) last :basis)))
      (is (empty? (store/submit-history db))))))
...
(deftest tin-verification-unverified-is-held-and-unoverridable
  (testing "unverified Nauru Revenue Office TIN registration when required -> HARD hold"
    (let [[db actor] (fresh)
      ...
      (is (= :hold (get-in res [:state :disposition])) "settles immediately, no interrupt")
      (is (not= :interrupted (:status res)))
      (is (some #{:tin-verification-unverified} (-> (store/ledger db) last :basis)))
      (is (empty? (store/submit-history db))))))
