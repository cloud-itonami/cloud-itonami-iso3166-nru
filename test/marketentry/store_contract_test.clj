...
(defn- exercise [s]
  (store/commit-record! s {:effect :engagement/upsert
                           :value {:id "eng-x" :operator "X LLC" :jurisdiction "NRU"
  ...
  (store/commit-record! s {:effect :assessment/set
                           :path ["eng-x"]
                           :payload {:jurisdiction "NRU" :checklist ["a"] :spec-basis "x"}})
...
(deftest mem-and-datomic-parity
  (let [mem (store/seed-db)
    ...
    (is (= (:conducted-via-procurement-agent? (:engagement m)) (:conducted-via-procurement-agent? (:engagement d))))
    (is (= (:procurement-agent (:engagement m)) (:procurement-agent (:engagement d))))
    (is (= (:split-group (:engagement m)) (:split-group (:engagement d))))
