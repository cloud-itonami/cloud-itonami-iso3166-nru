(ns marketentry.marketentryllm
  ...
  the Nauru (NRU) public-sector market-entry compliance actor.
...
(defn- propose-submit
  ...
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向けポータル提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "contract-value=" (:contract-value e)
                        " foreign-or-international-funded?=" (:foreign-or-international-funded? e)
                        " conducted-via-procurement-agent?=" (:conducted-via-procurement-agent? e)
                        " procurement-agent=" (:procurement-agent e)
                        " split-group=" (:split-group e)
     ...
     :confidence (if (and e
                          (or (not (:requires-tin-verification? e))
                              (:tin-verified? e)))
                   0.9 0.3)}))
