(ns marketentry.sim
  ...
  procurement-agent-requirement-mismatch and cross-entity
  artificial-division-violation checks."
...
(defn -main [& _]
  (let [db (store/seed-db)
        actor (op/build db)]
    (println "== engagement/intake eng-1 (NRU, clean) ==")
    ...
    (println "== jurisdiction/assess eng-4 (sets up procurement-agent-requirement-mismatch) ==")
    ...
    (println "== filing/submit eng-4 (procurement-agent-requirement-mismatch -> HARD hold) ==")
    (println (exec-op actor "t9" {:op :filing/submit :subject "eng-4"} operator))

    (println "== jurisdiction/assess eng-5 (sets up tin-verification-unverified) ==")
    ...
    (println "== filing/submit eng-5 (tin-verification-unverified -> HARD hold) ==")
    (println (exec-op actor "t11" {:op :filing/submit :subject "eng-5"} operator))

    (println "== jurisdiction/assess + filing/draft eng-6 and eng-7 (same split-group -- sets up artificial-division-violation) ==")
    ...
    (println (exec-op actor "t13" {:op :jurisdiction/assess :subject "eng-7"} operator))
    (println (approve! actor "t13"))
    (println (exec-op actor "t13b" {:op :filing/draft :subject "eng-7"} operator))
    (println (approve! actor "t13b"))
    (println "== filing/submit eng-6 (artificial-division-violation -> HARD hold) ==")
    (println (exec-op actor "t14" {:op :filing/submit :subject "eng-6"} operator))
